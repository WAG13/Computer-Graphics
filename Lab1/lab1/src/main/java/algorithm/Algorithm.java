...
public class Algorithm {
    ...
    public static boolean pointLocatesInSimplePolygon(Polygon p, Point z){
        ...
        for (int i = 0; i < p.getEdges().size(); i++){
            Edge currentEdge = p.getEdges().get(i);
            if (checkedEdges[i]){
                continue;
            }
            ...
            else if (currentEdge.isCrossedByHorizontalLineWithOrdinate(z.getY()) == CrossingType.CROSS
            ...
            //case 4: degenerate case - line crosses polygon vertex
            else if(currentEdge.isCrossedByHorizontalLineWithOrdinate(z.getY()) == CrossingType.CROSS_IN_VERTEX
                    && currentEdge.getXOfCrossingEdgeWithHorizontalLineThroughPoint(z.getY()) <= z.getX()){

                //find adjacent edge
                Edge edgeAdjacent = null;
                ...
                if (edgeCrossesLine(currentEdge, z, new Point(vertex.getX(), vertex.getY() - Constants.EPS)) &&
                        edgeCrossesLine(edgeAdjacent, z, new Point(vertex.getX(), vertex.getY() - Constants.EPS))){
                    leftCrossings += 2; //case must be filtered but no changes have to be done
                }
                else if(edgeCrossesLine(currentEdge, z, new Point(vertex.getX(), vertex.getY() - Constants.EPS)) ||
                        edgeCrossesLine(edgeAdjacent, z, new Point(vertex.getX(), vertex.getY() - Constants.EPS))){
                    leftCrossings += 1;
                }
     ...
     */
    public static boolean edgeCrossesLine(Edge edge, Point b, Point e){
