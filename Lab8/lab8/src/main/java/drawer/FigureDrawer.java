    ...
    public FigureDrawer(String title, ArrayList<Point> inputPoints, ArrayList<Point> pAnswer) {
        ...
        JPanel panel = new JPanel(){
            ...
            protected void paintComponent(Graphics g){
                ...
                g2 = (Graphics2D) g;
                g2.translate(screenWidth / 5,screenHeight * 4 / 5 );
                g2.scale(3.0, -3.0);
                ...
                for (int i = 0; i < inputPoints.size(); i++) {
                    Point v = inputPoints.get(i);
                    Point u = (i+1 == inputPoints.size()) ? inputPoints.get(0) : inputPoints.get(i+1);
                    edges.add(new Line2D.Double(v.getX()*SCALE, v.getY()*SCALE, u.getX()*SCALE, u.getY()*SCALE));
                }

                ArrayList<Line2D> hullEdges = new ArrayList<>();
                for (int i = 0; i < pAnswer.size(); i++) {
                    Point v = pAnswer.get(i);
                    Point u = (i+1 == pAnswer.size()) ? pAnswer.get(0) : pAnswer.get(i+1);
                    hullEdges.add(new Line2D.Double(v.getX()*SCALE, v.getY()*SCALE, u.getX()*SCALE, u.getY()*SCALE));
                }
